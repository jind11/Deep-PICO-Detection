...
def adversarial_loss(embedded, loss, loss_fn, perturb_norm_length):
  ...
  perturb = _scale_l2_dim4(grad, perturb_norm_length)
  return loss_fn(embedded + perturb)


def virtual_adversarial_loss(logits, embedded, num_classes, sentence_length, document_length,
                             logits_from_embedding_fn, perturb_norm_length):
  ...
  for _ in xrange(num_power_iteration):
    d = _scale_l2_dim4(
        _mask_by_length_4D(d, sentence_length), small_constant_for_finite_diff)

    d_logits = logits_from_embedding_fn(embedded + d, sentence_length, document_length)
  ...
  perturb = _scale_l2_dim4(d, perturb_norm_length)
  vadv_logits = logits_from_embedding_fn(embedded + perturb, sentence_length, document_length)
...
def virtual_adversarial_loss_bidir(logits, embedded, inputs,
  ...
  for _ in xrange(num_power_iteration):
    perturbs = [
        _scale_l2(d, small_constant_for_finite_diff) for d in perturbs
    ]
...
def _mask_by_length_4D(t, length):
  """Mask t, 4-D [batch, time1, time2, dim], by length, 2-D [batch, time1]."""
  maxlen = t.get_shape().as_list()[2]

  mask = tf.sequence_mask(length, maxlen=maxlen)
  mask = tf.expand_dims(tf.cast(mask, tf.float32), -1)
  # shape(mask) = (batch, num_timesteps, num_timesteps, 1)
  return t * mask
...
def _scale_l2_dim4(x, norm_length):
  # shape(x) = (batch, num_timesteps, num_timesteps, d)
  # Divide x by max(abs(x)) for a numerically stable L2 norm.
  # 2norm(x) = a * 2norm(x/a)
  # Scale over the full sequence, dims (1, 2, 3)
  alpha = tf.reduce_max(tf.abs(x), (1, 2, 3), keepdims=True) + 1e-12
  l2_norm = alpha * tf.sqrt(
      tf.reduce_sum(tf.pow(x / alpha, 2), (1, 2, 3), keepdims=True) + 1e-6)
  x_unit = x / l2_norm
  return norm_length * x_unit
...
def _kl_divergence_with_logits(q_logits, p_logits, num_classes):
  ...
  loss = tf.identity(tf.reduce_mean(kl), name='kl')
  return loss